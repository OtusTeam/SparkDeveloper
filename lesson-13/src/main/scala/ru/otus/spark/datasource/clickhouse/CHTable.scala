 ...
 * @param properties
 */
class CHTable(schema: StructType, partitioning: Array[Transform], properties: util.Map[String, String]) extends SupportsRead {
   ...
   * @return
   */
  override def newScanBuilder(options: CaseInsensitiveStringMap): ScanBuilder = new CHScanBuilder(schema, properties, options)
