...
import org.apache.spark.sql.functions.{array_contains, col, dayofweek, explode, struct}
...
object DataFrameAPIExercises extends SparkSessionWrapper {
  ...
  def ex1(): Unit = {
    ...
    val resDf = studentData.where(array_contains(col("skills"), "Python"))
    resDf.printSchema()
    resDf.show
  ...
  def ex2(): Unit = {
    ...
    // НАПИШИТЕ СВОЕ РЕШЕНИЕ ЗДЕСЬ
    val resDf = salesData.withColumn("order_item", explode(col("order_items")))
      .groupBy("order_item.product_name")
      .agg(functions.sum("order_item.quantity") as "total_amount")
  ...
  def ex3(): Unit = {
    ...
    // НАПИШИТЕ СВОЕ РЕШЕНИЕ ЗДЕСЬ
    val resDf = eventData.withColumn("day_of_week", dayofweek(col("event_date")))
    resDf.printSchema
    resDf.show
